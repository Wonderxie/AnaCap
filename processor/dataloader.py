...
def load_HDFS(log_file, label_file=None, window='session', train_ratio=0.5, split_type='sequential',
    ...
    if log_file.endswith('.csv'):
        ...
        for idx, row in struct_log.iterrows():
            blkId = row['No.']
            data_dict[blkId] = []
            hexArrTmp = re.findall(r'(\b0x[0-9a-fA-F]+\b)', row['Info'])
            for hexTmp in hexArrTmp:
                data_dict[blkId].append(hexTmp)
        ...
        if label_file:
            ...
            data_df['Label'] = data_df['BlockId'].apply(lambda x: 1 if label_dict[x] == 1 else 0)
